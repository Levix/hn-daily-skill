...
test('runOpenClawAgent reads text from payloads array when output_text is absent', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'hn-daily-openclaw-payloads-'));
  const binDir = join(tempRoot, 'bin');
  const openclawPath = join(binDir, 'openclaw');
...
test('runOpenClawAgent reads text from result.payloads when payloads are nested under result', async () => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'hn-daily-openclaw-result-payloads-'));
  const binDir = join(tempRoot, 'bin');
  const openclawPath = join(binDir, 'openclaw');
  ...
  try {
    ...
    await writeFile(openclawPath, `#!/bin/zsh
cat <<'JSON'
{"status":"ok","result":{"payloads":[{"text":"中文标题: 标题\\n一句话总结: 这是足够长的一句话总结。\\n详细摘要: 这是详细摘要，长度足够，能够通过基础校验。\\n关键要点:\\n- 要点一\\n- 要点二\\n- 要点三\\n- 要点四\\n- 要点五\\n技术洞察: 技术洞察内容。\\n为什么它火了: 社区讨论充分。\\n标签: #AI #Infra"}]}}
JSON
`, { mode: 0o755 });
